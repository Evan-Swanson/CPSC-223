...
int main()
{
   ...
   for (numberOfItems = 1000; numberOfItems < 10000; numberOfItems += 1000) {  
      if (option == 1)
	  ...
	  cout << "# of operations for HeapSort       = " << numberOperationsHeapSort << endl;
...
bool isALeaf(int rootIndex, int keysCount, int& numops)
{
	int leftChildIndex = 2 * rootIndex + 1; numops += 3;
	numops += 2;
...
{ 
	...
	if (not isALeaf(rootIndex, keysCount, numops))
	{
		...
		{
			swap(heap[rootIndex], heap[largerChildIndex]); numops += 7;
