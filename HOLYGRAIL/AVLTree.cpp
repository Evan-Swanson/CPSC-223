...
//data object: a binary search tree which is either empty or 
//                  in the form of     r
//                                  /    \
//                                TL     TR
//                  where TL and TR are binary search trees
// 					and |Height of TR - Height of TL| <= 1
...
{
	for(int k = start; k < MAXHEIGHT - 1; k++) 
	{
...
//performs a single left rotation to maintain tree balance
//pre: pivot points to the pivot node, root points to the root
//post: subtree is rotated left once about the pivot node
//usage: doSingleLeftRotation(search.path[search.pivotIndex], search, root);
void doSingleLeftRotation(TreeNode*& pivot, SearchPath& search, TreeNode*& root)
{
	TreeNode* tempPtr;
	tempPtr = pivot -> rightChild;
	pivot -> rightChild = pivot -> rightChild -> leftChild;
	tempPtr -> leftChild = pivot;
	
	if(pivot == search.path[0]) 	//root
	{
		root = tempPtr;
	}
	
	else if(search.path[search.pivotIndex - 1] -> rightChild == pivot)	 //rightChild
	{
		search.path[search.pivotIndex - 1] -> rightChild = tempPtr;
	}		
	
	else			//leftChild
	{
		search.path[search.pivotIndex - 1] -> leftChild = tempPtr;
	}

	
}

//performs a single right rotation to maintain tree balance
//pre: pivot points to the pivot node, root points to the root
//post: subtree is rotated right once about the pivot node
//usage: doSingleRightRotation(search.path[search.pivotIndex], search, root);
void doSingleRightRotation(TreeNode*& pivot, SearchPath& search, TreeNode*& root)
{
	TreeNode* tempPtr;
	tempPtr = pivot -> leftChild;
	pivot -> leftChild = pivot -> leftChild -> rightChild;
	tempPtr -> rightChild = pivot;
...
//performs a double left right rotation to maintain tree balance
//pre: pivot points to the pivot node, root points to the root
//post: subtree is rotated such that the grandchild of the pivot's children is now
//		child of pivot and pivot
//usage: doDoubleLeftRightRotation(search.path[search.pivotIndex], search, root);
void doDoubleLeftRightRotation(TreeNode*& pivot, SearchPath& search, TreeNode*& root)
{
	TreeNode* tempChildPtr = pivot -> leftChild;
	TreeNode* tempGrandChildPtr = pivot -> leftChild -> rightChild;
	
	pivot -> leftChild = tempGrandChildPtr -> rightChild;
	tempChildPtr -> rightChild = tempGrandChildPtr -> leftChild;
	tempGrandChildPtr -> leftChild = tempChildPtr;
	tempGrandChildPtr -> rightChild = pivot;
	
	if(pivot == search.path[0]) 	//root
		root = tempGrandChildPtr;
	
	else if(search.path[search.pivotIndex - 1] -> rightChild == pivot)	 //rightChild
		search.path[search.pivotIndex - 1] -> rightChild = tempGrandChildPtr;		
	
	else   //leftChild
		search.path[search.pivotIndex - 1] -> leftChild = tempGrandChildPtr;
		
}

//performs a double right left rotation to maintain tree balance
//pre: pivot points to the pivot node, root points to the root
//post: subtree is rotated such that the grandchild of the pivot's children is now
//		child of pivot and pivot
//usage: doDoubleRightLeftRotation(search.path[search.pivotIndex], search, root);
void doDoubleRightLeftRotation(TreeNode*& pivot, SearchPath& search, TreeNode*& root)
{
	
	TreeNode* tempChildPtr = pivot -> rightChild;
	TreeNode* tempGrandChildPtr = pivot -> rightChild -> leftChild;
	
	pivot -> rightChild = tempGrandChildPtr -> leftChild;
	tempChildPtr -> leftChild = tempGrandChildPtr -> rightChild;
	tempGrandChildPtr -> rightChild = tempChildPtr;
	tempGrandChildPtr -> leftChild = pivot;
	
	if(pivot == search.path[0]) 	//root
		root = tempGrandChildPtr;
	
	else if(search.path[search.pivotIndex - 1] -> leftChild == pivot)	 //rightChild
		search.path[search.pivotIndex - 1] -> leftChild = tempGrandChildPtr;		
...
//post: newItem has been inserted into the AVL tree and the tree
//  	has been adjusted if needed to maintain balance
// 		An exception is thrown if there is not enough room in heapcheck
// 		or the item already exists in the tree
...
void AVLTree::insert(const Item& newItem) throw (Exception)
{
   ...
   {
      doSingleLeftRotation(search.path[search.pivotIndex], search, root);
	  fixBalances(search, search.pivotIndex + 2);
	  search.path[search.pivotIndex] -> balance = 0;
   }
   else if (isSingleRotateRight(search))
   {
      doSingleRightRotation(search.path[search.pivotIndex], search, root);
	  fixBalances(search, search.pivotIndex + 2);
	  search.path[search.pivotIndex] -> balance = 0;
   }
   else if (isDoubleRotateLeftRight(search))
   {
      doDoubleLeftRightRotation(search.path[search.pivotIndex], search, root);
	  fixBalances(search, search.pivotIndex + 3);
	  search.path[search.pivotIndex] -> balance = 1;
   }
  else
  {
     doDoubleRightLeftRotation(search.path[search.pivotIndex], search, root);
 	 fixBalances(search, search.pivotIndex + 3);
	 search.path[search.pivotIndex + 1] -> balance = 1;
	 search.path[search.pivotIndex] -> balance = 0;
